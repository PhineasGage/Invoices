...
public class InFileCompanyDatabase<T extends ItemsForDatabase> extends InFileDatabase<T> {

  private FileHelper<T> fileHelper;
  private ItemConverter<T> itemConverter;
  private IndexHelper<T> indexHelper;

  public InFileCompanyDatabase(FileHelper fileHelper,
  ...
  public int saveItem(T item) {
    Optional<T> optionalCompany = this
        .getItems()
        .stream()
        .filter(n -> ((Company) n).getNip().equals(((Company) item).getNip()))
        .findFirst();
  ...
  public void updateItemById(int id, T item) {
    List<T> itemList = getAllItemsExceptCompanyWithSpecifiedId(id);
    T itemCopy = (T) item.makeDeepCopy(item);
    itemCopy.setId(id);
  ...
  private List<T> getAllItemsExceptCompanyWithSpecifiedId(int id) {
    return getItems()
        .stream()
        .filter(n -> n.getId() != id)
